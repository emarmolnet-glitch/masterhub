...
test('10. AgenteProyectosWidget injects dynamic projectContext and systemInstruction into Gemini API payload', () => {
  ...
  assert.match(widgetSource, /const\s+systemInstruction\s*=\s*`Eres el Agente de Proyectos de MasterHub Core PRO, impulsado por Gemini\. Eres un consultor estratégico marítimo y un socio conversacional altamente inteligente\.[\s\S]*?REGLA CERO - SALUDOS Y MENSAJES CASUALES:[\s\S]*?REGLAS DE COMPORTAMIENTO Y PERSONALIDAD:[\s\S]*?Contexto actual del proyecto: \$\{projectContext\}`/);
